...
public class LongArrayCodec implements MemCodec<long[]> {
    private final int length;

    public LongArrayCodec(int length) {
        this.length = length;
    }

    @Override
    public long[] decode(@NotNull MemorySegment segment, long offset) {
        return segment.asSlice(offset, (long) length * Long.BYTES).toArray(ValueLayout.JAVA_LONG);
    }

    @Override
    public void encode(Arena arena, MemorySegment segment, long offset, long[] element) {
        ...
        segment.asSlice(offset, (long) length * Long.BYTES).copyFrom(MemorySegment.ofArray(element));
    }
